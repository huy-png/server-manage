...
import paramiko
from PySide6.QtCore import QObject, QRunnable, QThreadPool, Qt, QTimer, Signal, QPointF, QRectF
...
# The SSD browser is deliberately restricted to the SSH user's actual home
# directory. Its absolute path is resolved after login instead of assuming
# /home/<username>, which does not exist on every server.
...
# A small fixed cap improves batches of small files without overwhelming the
# home network, server disk, or SSH daemon with too many SFTP connections.
...
def fmt_duration(seconds: float) -> str:
    ...
    if hours: return f"{hours} giờ {minutes} phút"
    if minutes: return f"{minutes} phút {seconds} giây"
...
def parse_status(output: str) -> dict:
    result: dict = {"errorLog": [], "failedUnitNames": [], "activeUsers": []}; reading_errors = reading_failed_units = reading_users = False
...
class WorkerSignals(QObject):
    done = Signal(object); error = Signal(str); progress = Signal(object)


class Worker(QRunnable):
    def __init__(self, task: Callable):
        super().__init__(); self.task = task; self.signals = WorkerSignals()
    def run(self):
        try:
            result = self.task()
        except Exception as error:
            # The app can be closed while an SSH/PowerShell task is still
            # finishing. In that case Qt has already destroyed the receiver.
            try: self.signals.error.emit(str(error))
